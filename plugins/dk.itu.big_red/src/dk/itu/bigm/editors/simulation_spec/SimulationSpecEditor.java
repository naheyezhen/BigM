...
public class SimulationSpecEditor extends AbstractNonGEFEditor implements
		PropertyChangeListener {
	private static class ExportInteractionManagerFactory extends
			ConfigurationElementInteractionManagerFactory {
		...
		public IInteractionManager newInstance() {
			...
			try {
				if (getCE().createExecutableExtension("class") instanceof Saver) {
					...
					return (InteractionManager) getCE()
	...
	private static final IFile getFileFrom(ModelObject m) {
		...
		return (fw instanceof EclipseFileWrapper ? ((EclipseFileWrapper) fw)
				.getResource() : null);
	}
	...
	private static ArrayList<IFactory<IInteractionManager>> getIMFactories() {
		ArrayList<IFactory<IInteractionManager>> factories = new ArrayList<IFactory<IInteractionManager>>();

		IExtensionRegistry r = RegistryFactory.getRegistry();
		for (IConfigurationElement ce : r
				.getConfigurationElementsFor(IInteractionManager.EXTENSION_POINT))
			factories
					.add(new ConfigurationElementInteractionManagerFactory(ce));
	...
	private void recalculateExportEnabled() {
		export.setEnabled(getModel().getModel() != null
	...
	public void createEditorControl(Composite parent) {
		...
		signatureSelector.addListener(new ResourceListener() {
			...
			public void resourceChanged(IResource oldValue, IResource newValue) {
				if (uiUpdateInProgress)
					return;
				try {
					Signature s = (newValue != null ? (Signature) new EclipseFileWrapper(
							(IFile) newValue).load() : null);
					doChange(new BoundDescriptor(getModel(),
							new SimulationSpec.ChangeSetSignatureDescriptor(
									new SimulationSpec.Identifier(), getModel()
											.getSignature(), s)));
					recalculateExportEnabled();
		...
		brl.marginBottom = brl.marginLeft = brl.marginRight = brl.marginTop = 0;
		brl.pack = false;
		br.setLayout(brl);
		...
		StockButton.ADD.create(br, SWT.NONE, true).addSelectionListener(
				new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						ResourceSelectionDialog rtsd = new ResourceSelectionDialog(
								getSite().getShell(),
		...
		modelSelector.addListener(new ResourceListener() {
			...
			public void resourceChanged(IResource oldValue, IResource newValue) {
				if (uiUpdateInProgress)
					return;
				try {
					Bigraph b = (newValue != null ? (Bigraph) new EclipseFileWrapper(
							(IFile) newValue).load() : null);
					doChange(new BoundDescriptor(getModel(),
		...
		new Label(self, SWT.HORIZONTAL | SWT.SEPARATOR)
		...
		export.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				IInteractionManager im = (IInteractionManager) ((IFactory<?>) ((IStructuredSelection) cv
						.getSelection()).getFirstElement()).newInstance();
