...
import it.uniud.bigmit.command.CreateCommand;
...
import it.uniud.bigmit.figure.NestedBigraphFigure;
import it.uniud.bigmit.figure.ReactionFiguren;
...
import dk.itu.bigm.editors.bigraph.parts.RootPart;
		...
		if ( child instanceof GraphPart ) {
			...
			if ( target.getGraph() != null ) {
				return new CompositionCommand( ( Graph )child.getModel(), target.getGraph(),
						                       ( ( BigmitRootEditPart )child.getRoot() ).getWorkbenchPart() );
			}
		}
