...
public class BigraphXMLSaver extends XMLSaver {
	...
	public Element processModel(Element e) throws SaveFailedException {
		...
		for (Layoutable i : obj.getChildren()) {
			...
			if (i instanceof Edge) {
				edges.add(f = processLinkType(newElement(BIGRAPH, "bigraph:edge"), (Link)i));
			} else if (i instanceof OuterName) {
				outernames.add(f = processLinkType(newElement(BIGRAPH, "bigraph:outername"), (Link)i));
				...
				innernames.add(f = processPoint(
						newElement(BIGRAPH, "bigraph:innername"), (InnerName)i));
			}
	...
	private static Element processPoint(Element e, Point p)
		...
		if (p instanceof InnerName) {
			applyAttributes(e, "innersort", ((InnerName)p).getInnerSort() == null ? "none" : ((InnerName)p).getInnerSort());
		}
		return e;
	}
	
	private static Element processLinkType(Element e, Link l)
			throws SaveFailedException {
		if (l != null) {
			applyAttributes(e, "name", l.getName());
			applyAttributes(e, "linksort", l.getLinkSort() == null ? "none" : l.getLinkSort());
			return e;
		} else return null;
	}
