...
public abstract class XMLSaver extends Saver implements IXMLSaver {
	...
	protected boolean defNSMatch(String nsURI) {
		return useDefaultNamespace && defaultNamespace != null && nsURI != null
	...
	protected Element newElement(String nsURI, String qualifiedName) {
		if (defNSMatch(nsURI)) {
			return getDocument().createElementNS(nsURI,
					unqualifyName(qualifiedName));
		} else
			return getDocument().createElementNS(nsURI, qualifiedName);
	}
	...
	protected Element processOrReference(Element e, ModelObject object,
			XMLSaver ex) throws SaveFailedException {
		IFileWrapper f;
		if (e == null || object == null) {
			return null;
		} else if (getFile() != null && (f = FileData.getFile(object)) != null) {
			e.setAttributeNS(null, "src",
					f.getRelativePath(getFile().getParent().getPath()));
	 ...
	 * attributes. (This uses {@link Element#setAttribute}, but is slightly less
	 * irritating, as it automatically converts names and values to strings.)
	 * 
	 * @param d
	 *            an Element
	 * @param attrs
	 *            a vararg list of name-value pairs of any type
	...
	protected static Element applyAttributes(Element d, Object... attrs) {
		for (int i = 0; i < attrs.length; i += 2) {
			...
			if (attrName != null && attrValue != null) {
				if (attrName.equals("name") && attrValue.equals("_")) {// controlÊÇÀà
					// do nothing
				} else {
	 ...
	 * 
	 * @param e
	 *            the would-be parent of the new node
	 * @param newChild
	 *            the node to add
	 */
	 ...
	 * 
	 * @param ns
	 *            the namespace URI of the document to create
	 * @param qName
	...
	protected Document createDocument(String ns, String qName) {
		...
		if (defNSMatch(ns)) {
			...
			return impl.createDocument(ns, qName, null);
	}
